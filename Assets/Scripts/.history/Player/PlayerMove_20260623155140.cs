...
public class EldenRingMovement : MonoBehaviour
{
    ...
    public void TriggerHitStop() => _health?.TriggerHitStop();
    ...
    /// <summary>生命系统（从主脚本拆出到 PlayerHealth）</summary>
    ...
    public Vector3 impact { get => _health != null ? _health.impact : Vector3.zero; set { if (_health != null) _health.impact = value; } }
    ...
    private bool nextAttackIsCrit { get => _health != null && _health.NextAttackIsCrit; set { if (_health != null) _health.NextAttackIsCrit = value; } }
    private bool nextHeavyAttackIsFourth { get => _health != null && _health.NextHeavyAttackIsFourth; set { if (_health != null) _health.NextHeavyAttackIsFourth = value; } }
    ...
    void Start()
    {
        ...
        if (_health != null)
        {
            ...
            _health.getForward        = () => transform.forward;
            _health.onRageGain         = (amount) => { currentRage += amount; };

            // ── 每帧同步数值（PlayerStatsManager 在别处更新） ──
            // 受击/死亡事件 → 只设状态机和动画，不解耦处继续用 _health 读血量
            _health.OnDamageTaken += (knockbackDir) =>
            {
    ...
    // 闪避无敌开始（动画事件调用）
    public void OnDodgeInvincibleStart() { isInvincible = true; dodgeStartTime = Time.time; }
    // 闪避无敌结束
    public void OnDodgeInvincibleEnd() { isInvincible = false; staminaRegenBuffTimer = 1.5f; }
